import Character1 from './1.png';
import Character2 from './2.png';
import Character3 from './3.png';
import Character4 from './4.png';
import Character5 from './5.png';
import Character6 from './6.png';
import Character7 from './7.png';
import Character8 from './8.png';
import Character9 from './9.png';
import Character10 from './10.png';

export const Character = [
    { index: 1, imgSrc: Character1 },
    { index: 2, imgSrc: Character2 },
    { index: 3, imgSrc: Character3 },
    { index: 4, imgSrc: Character4 },
    { index: 5, imgSrc: Character5 },
    { index: 6, imgSrc: Character6 },
    { index: 7, imgSrc: Character7 },
    { index: 8, imgSrc: Character8 },
    { index: 9, imgSrc: Character9 },
    { index: 10, imgSrc: Character10 }
];  

export default  [
    Character1,
    Character2,
    Character3,
    Character4,
    Character5,
    Character6,
    Character7,
    Character8,
    Character9,
    Character10

];  
